    ...
    <el-dialog width="20%" title="收货地址" :visible.sync="dialogFormVisible">
        <el-form>
            <el-form-item label="选择大区" :label-width="formLabelWidth">
                <el-cascader :model="area" :options="options" @change="handleChange" :props="{ children: 'opt_data_array', value: 'v', label: 't' }"></el-cascader>
            </el-form-item>
        </el-form>
    ...
    function checkLogin() {
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.code === 200) {
                    switch (parseInt(login_data[0])) {
                        case 65: // 二维码已失效
                            app.login_status_msg = '二维码已过期'
                            clearInterval(interval_check_login)
                            break
                        case 66: // 二维码未失效
                            break
                        case 67: // 认证中
                            app.login_status_msg = '认证中'
                            break
                        case 68: // 登录拒绝
                            app.login_status_msg = '登录被拒绝'
                            clearInterval(interval_check_login)
                            break
                        default:
                            clearInterval(interval_check_login)
